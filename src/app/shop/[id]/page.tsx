...
export default function ProductPage() {
  ...
  const rawProduct = shopifyData.products.find(p => p.handle === params.id) || shopifyData.products[0];
  
  const productData = {
    id: rawProduct.handle,
    name: rawProduct.title,
    price: "$" + (rawProduct.variants?.[0]?.price || "0.00"),
    description: rawProduct.body_html.replace(/<[^>]+>/g, '') || "A premium Fleñjure product.",
    details: [
      "Official Fleñjure Quality",
      "Sourced with care",
      "Imported"
    ],
    sizes: rawProduct.options?.find(o => o.name === "Size" || o.name === "Title")?.values
      .filter(v => v !== "Default Title") || [],
    images: rawProduct.images?.map(img => img.src) || [
      "https://images.unsplash.com/photo-1556821840-0a63f95609a7?q=80&w=2000&auto=format&fit=crop"
    ]
  };
